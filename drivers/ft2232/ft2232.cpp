/***************************************************************************
*   Copyright (C) 2009 by 脴yvind Harboe                                   *
*	脴yvind Harboe <oyvind.harboe@zylin.com>                               *
*                                                                         *
...
#include "drivers/jtag/interface.h"
#include "drivers/jtag/commands.h"
#include "drivers/jtag/jtag.h"
...
int ft2232_write(uint8_t* buf, int size, uint32_t* bytes_written)
{
#if BUILD_FT2232_FTD2XX == 1
	FT_STATUS status;
...
int ft2232_read(uint8_t* buf, uint32_t size, uint32_t* bytes_read)
{
#if BUILD_FT2232_FTD2XX == 1
	DWORD dw_bytes_read;
	FT_STATUS status;
	...
	while (cmd != last)
	{
		...
		case JTAG_SCAN:
			...
			if (type != SCAN_OUT)
			{
				...
				buffer    = (unsigned char*)calloc(DIV_ROUND_UP(scan_size, 8), 1);
...
static int ft2232_large_scan(struct scan_command* cmd, enum scan_type type, uint8_t* buffer, int scan_size)
{
	...
	int cur_byte  = 0;
	bool last_bit;
	uint8_t* receive_buffer  = (uint8_t*)malloc(DIV_ROUND_UP(scan_size, 8));
...
static int ft2232_init_ftd2xx(void)
{
	...
	else
	{
		...
		LOG_INFO("deviceID: 0x%08x", deviceID);
...
static int ft2232_init_ftd2xx_0(uint16_t vid, uint16_t pid, int more, int* try_more)
{
	FT_STATUS	status;
	DWORD		deviceID;
	char		SerialNumber[16];
...
static int ft2232_init_libftdi(uint16_t vid, uint16_t pid, int more, int* try_more)
{
	...
	{
		if (more)
			printf("unable to open ftdi device (trying more): %s", ftdic.error_str);
		else
			printf("unable to open ftdi device: %s", ftdic.error_str);
		*try_more = 1;
		return ERROR_JTAG_INIT_FAILED;
	}
...
int ft2232_init(void)
{
	...
	if ((ft2232_layout == NULL) || (ft2232_layout[0] == 0))
	{
		...
		//LOG_WARNING("No ft2232 layout specified, using default 'jtagkey'");
	}
...
//				more, &try_more);
	...
	ft2232_speed(jtag_speed); /* FT2232D:  speed = 12MHz/(jtag_speed + 1)*/
	                          /* FT2232H:  speed = 5*12MHz/(jtag_speed + 1)*/ 
