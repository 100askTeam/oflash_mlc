...
static u8 pcbuf_read8(u8 *addr)
{
	return *addr;
}

static u16 pcbuf_read16(u16 *addr)
{
	return *addr;
}

static u32 pcbuf_read32(u32 *addr)
{
	return *addr;
}

static u64 pcbuf_read64(u64 *addr)
{
	return *addr;
}
...
{
	if (sect != (flash_sect_t)(info->sector_count - 1))
...
static inline uchar flash_read_uchar (flash_info_t * info, uint offset)
{
	...
	cp = (uchar *)flash_map (info, 0, offset);
#if defined(__LITTLE_ENDIAN) || defined(CFG_WRITE_SWAPPED_DATA)
...
static inline ushort flash_read_word (flash_info_t * info, uint offset)
{
	...
	addr = (ushort *)flash_map (info, 0, offset);
			      ...
			      uint offset)
{
	...
	int x;
#endif
	addr = (uchar *)flash_map (info, sect, offset);
...
#else
	return 0; /* not needed at JTAG access */
#endif
}
				  ...
				  int len)
{
	...
	int retcode;
	u8 *src = cp;
	u8 *dst = (u8 *)map_physmem(dest, len, MAP_NOCACHE);
	u8 *dst2 = dst;
	int flag = 0;
	uint offset = 0;
	unsigned int shift;
	...
	case CFI_CMDSET_INTEL_EXTENDED:
		...
		retcode = flash_status_check (info, sector,
					      ...
					      "write to buffer");
#else
		retcode = 0;	/* not needed at JTAG access */
#endif
		if (retcode == ERR_OK) {
			...
			while (cnt-- > 0) {
				switch (info->portwidth) {
				case FLASH_CFI_8BIT:
					flash_write8(pcbuf_read8(src), dst);
					src += 1, dst += 1;
					break;
				case FLASH_CFI_16BIT:
					flash_write16(pcbuf_read16((u16 *)src), dst);
					src += 2, dst += 2;
					break;
				case FLASH_CFI_32BIT:
					flash_write32(pcbuf_read32((u32 *)src), dst);
					src += 4, dst += 4;
					break;
				case FLASH_CFI_64BIT:
					flash_write64(pcbuf_read64((u64 *)src), dst);
					src += 8, dst += 8;
					break;
				default:
					retcode = ERR_INVAL;
...
int flash_erase (flash_info_t * info, int s_first, int s_last)
{
	...
	prot = 0;
	for (sect = (ulong)s_first; sect <= (ulong)s_last; ++sect) {
		if (info->protect[sect]) {
			prot++;
		}
	}
	if (prot) {
		fprintf(stderr, "- Warning: %d protected sectors will not be erased!\n",
			prot);
	} 
...
int write_buff (flash_info_t * info, uchar * src, ulong addr, ulong cnt)
{
	...
	if ((aln = addr - wp) != 0) {
		cword.l = 0;
		p = (u8 *)map_physmem(wp, info->portwidth, MAP_NOCACHE);
		for (i = 0; i < aln; ++i)
	...
	while (cnt >= info->portwidth) {
		...
		percent = (size - cnt)*100 / size;
		if (percent != prepercent)
		{
			prepercent = percent;
			fprintf(stderr, "\r%02d%%", percent);
		}
		
		/* prohibit buffer write when buffer_size is 1 */
		if (info->buffer_size ==0 || info->buffer_size == 1) {
		//if (1) { // write buffer 功能仍需验证暂时屏蔽, thisway.diy
			cword.l = 0;
	 ...
	 */
	//fprintf(stderr, "\nhandle unaligned tail bytes: data = 0x%x, info->portwidth = %d, cnt = %d, wp = %d\n", *src, info->portwidth, cnt, wp);
	cword.l = 0;
	p = (u8 *)map_physmem(wp, info->portwidth, MAP_NOCACHE);
...
//		flash_add_byte (info, &cword, flash_read8(p + i));
		flash_add_byte (info, &cword, 0xff);
	}
	unmap_physmem(p, info->portwidth);
...
{	
	info->portwidth = CFG_FLASH_CFI_PORT_WIDTH;
	info->chipwidth = CFG_FLASH_CFI_CHIP_WIDTH;
...
ulong flash_get_size (ulong base, int banknum)
{
	...
	if (flash_detect_cfi (info, &qry)) {
		...
		info->sector_count = (unsigned short)sect_cnt;
...
unsigned long bank_base[CFI_MAX_FLASH_BANKS] = CFG_FLASH_BANKS_LIST;
#define BANK_BASE(i)	bank_base[i]
