...
struct amd_flash_info {
	...
	__u16 dev_id;
	char *name;
	__u8 uaddr[4];		/* unlock addrs for 8, 16, 32, 64 */
	int DevSize;
	int CmdSet;
	int NumEraseRegions;
	ulong regions[16];
...
//	debug("Found JEDEC Flash: %s\n", jedec_entry->name);
	...
	case FLASH_CFI_8BIT:
		uaddr_idx = (enum uaddr)jedec_entry->uaddr[0];
		break;
	case FLASH_CFI_16BIT:
		uaddr_idx = (enum uaddr)jedec_entry->uaddr[1];
		break;
	case FLASH_CFI_32BIT:
		uaddr_idx = (enum uaddr)jedec_entry->uaddr[2];
		break;
	default:
...
//	debug("unlock addresses are 0x%x/0x%x\n", info->addr_unlock1, info->addr_unlock2);
	...
	for (i = 0; i < jedec_entry->NumEraseRegions; i++) {
		...
		for (j = 0; j < (int)erase_region_count; j++) {
