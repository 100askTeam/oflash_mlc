...
{
    if(addr<0x8000000)
	S2440_SetPin(nGCS0 ,LOW);
    else if(addr<0x10000000)
	S2440_SetPin(nGCS1 ,LOW);
    else if(addr<0x18000000)
	S2440_SetPin(nGCS2 ,LOW);
    else if(addr<0x20000000)
	S2440_SetPin(nGCS3 ,LOW);
    else if(addr<0x28000000)
	S2440_SetPin(nGCS4 ,LOW);
    else if(addr<0x30000000)
	S2440_SetPin(nGCS5 ,LOW);
    else if(addr<0x38000000)
	S2440_SetPin(nGCS6 ,LOW);
    else if(addr<0x40000000)
	S2440_SetPin(nGCS7 ,LOW);
}
...
//	ft2232_clear_buf();
    ...
    if(en_nBE)
    {
	if(bs&(1<<0))S2440_SetPin(nBE0,LOW); 
	if(bs&(1<<1))S2440_SetPin(nBE1,LOW); 
    }
    JTAG_ShiftDRStateNoTdo(outCellValue); 
...
//	ft2232_clear_buf();
	...
	if(bs&(1<<0))S2440_SetPin(nBE0,LOW); 
	if(bs&(1<<1))S2440_SetPin(nBE1,LOW); 
	if(bs&(1<<2))S2440_SetPin(nBE2,LOW); 
	if(bs&(1<<3))S2440_SetPin(nBE3,LOW); 
    }
    JTAG_ShiftDRStateNoTdo(outCellValue); 
...
{
//	ft2232_clear_buf();
    S2440_SetPin(DATA0_7_CON ,LOW); 
    S2440_SetPin(DATA8_15_CON ,LOW); 
    S2440_SetPin(DATA16_23_CON ,LOW); 
    S2440_SetPin(DATA24_31_CON ,LOW); 
    S2440_SetAddr(addr);
    S2410_Assert_nGCS(addr);
    S2440_SetPin(nWE,LOW); 
...
//	ft2232_clear_buf();
    ...
    S2410_Assert_nGCS(addr);
    S2440_SetPin(nWE,LOW);
    if(bs&(1<<0))S2440_SetPin(nBE0,LOW); //used as nWBE/nBE
    if(bs&(1<<1))S2440_SetPin(nBE1,LOW); 
    ...
    if(!en_nBE)
    {
	S2440_SetPin(nBE0,HIGH); //nWBE is deasserted here.
	S2440_SetPin(nBE1,HIGH); 
    }
    JTAG_ShiftDRStateNoTdo(outCellValue); 

    S2440_SetPin(DATA0_7_CON ,HIGH); //nBE is deasserted here.
    S2440_SetPin(DATA8_15_CON ,HIGH); 
    S2440_SetPin(DATA16_23_CON ,HIGH); 
    S2440_SetPin(DATA24_31_CON ,HIGH); 
    S2410_Deassert_nGCS(addr);
...
//	ft2232_clear_buf();
    ...
    if(en_nBE)
    {
	S2440_SetPin(nBE0,HIGH); //nBE is deasserted here.
	S2440_SetPin(nBE1,HIGH); 
	S2440_SetPin(nBE2,HIGH); 
	S2440_SetPin(nBE3,HIGH); 
    }
...
//	ft2232_clear_buf();
    ...
    if(en_nBE)
    {
	if(bs&(1<<0))S2440_SetPin(nBE0,LOW); //used as nBE
	if(bs&(1<<1))S2440_SetPin(nBE1,LOW); 
    }
    S2440_SetDataHW(data);
    JTAG_ShiftDRStateNoTdo(outCellValue); //tCOS
    ...
    if(!en_nBE)
    {
	if(bs&(1<<0))S2440_SetPin(nBE0,LOW); //used as nWBE
	if(bs&(1<<1))S2440_SetPin(nBE1,LOW); //used as nWBE
    }
    S2440_SetPin(nWE,LOW);
    JTAG_ShiftDRStateNoTdo(outCellValue); 
...
struct jtag_mem_rdwr sc32440_jtag_mem_rdwr = {
	"S3C2440",
	0x0032409d,
	S2440_InitCell,
	MRW_Menu,
