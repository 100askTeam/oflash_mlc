    ...
    for (bus = usb_get_busses(); bus; bus = bus->next)
    {
        for (dev = bus->devices; dev; dev = dev->next)
        {
                    ...
                    && dev->descriptor.idProduct == product)
            {
				...
				if (usb_get_string_simple(ftdi->usb_dev, dev->descriptor.iManufacturer, string, sizeof(string)) <= 0)
				{
					usb_close (ftdi->usb_dev);
					ftdi_error_return(-7, "unable to fetch manufacturer description");
				}
				if (strncmp(string, manufacturer, sizeof(string)) != 0)
				{
					if (usb_close (ftdi->usb_dev) != 0)
